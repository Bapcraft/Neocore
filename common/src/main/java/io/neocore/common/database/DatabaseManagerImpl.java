...
public class DatabaseManagerImpl implements DatabaseManager {
	...
	private DatabaseController makeNewController(Class<? extends DatabaseController> clazz, Config cfg) {
		...
		NullPointerException noConstNpe = new NullPointerException("No constructor for class " + clazz.getName()
				+ " that takes a " + Config.class.getName() + " as the single argument.");
	...
	public void configure(DatabaseConfig config) {
		...
		if (this.configured)
			throw new IllegalStateException("Database manager already configured.");
		...
		for (DatabaseService serv : DatabaseService.values()) {
			...
			ctrlConfigs.put(ctrl, config.getControllerConfig(serv)); // Not the
																		// cleanest.
		...
		for (Map.Entry<Class<? extends DatabaseController>, List<DatabaseService>> entry : controllers.entrySet()) {
			...
			if (services.size() > 0) {
				...
				for (int i = 0; i < provs.length; i++) {
					...
					log.info("Service " + serv.getName() + " is provided by "
							+ (prov != null ? prov.getClass().getSimpleName() : "null"));
	...
	public void registerController(String name, Class<? extends DatabaseController> controller) {
		...
		if (this.dbTypes.containsKey(name)) {
			...
			throw new IllegalStateException("A database controller named " + name + " of type " + clazzName
					+ " already exists when registering " + repName);

		}

		NullPointerException noConstNpe = new NullPointerException("No constructor for class " + controller.getName()
				+ " that takes a " + Config.class.getName() + " as the single argument.");
