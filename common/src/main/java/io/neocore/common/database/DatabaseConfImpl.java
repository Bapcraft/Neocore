...
public class DatabaseConfImpl implements DatabaseConfig {
	...
	public DatabaseConfImpl(Config conf) {
		...
		for (String name : ctrls.keySet()) {
			...
			// Determine how we should pull the configuration for the DB
			// controller.
			if (ctrl.valueType() == ConfigValueType.OBJECT) {
				...
				throw new NullPointerException(
						"Invalid controller definition for " + name + "! (" + ctrl.origin().lineNumber() + ")");
			}
		...
		for (Entry<String, ConfigValue> e : provs.entrySet()) {
			...
			if (database.valueType() != ConfigValueType.STRING)
				throw new NullPointerException("Database definition for " + service + " is not a string!");

			String dbName = database.unwrapped().toString();
			if (!this.databaseConfigs.containsKey(dbName))
				throw new NullPointerException("Database name for " + service + " (\"" + dbName + "\") not found!");
	...
	public Config getControllerConfig(ServiceType type) {
		...
		if (this.databaseConfigs.containsKey(ctrlName)) {
			...
			throw new NullPointerException(
					"Service " + typeStr + " does not have a provider class defined and there is no default set!");
		}
