...
public abstract class NetworkSync {
	 ...
	 * 
	 * @param uuids
	 *            The set of player UUIDs on the server
	 */
	 ...
	 * 
	 * @param uuid
	 *            The UUID of the invalidated player
	 */
	public abstract void announceInvalidation(UUID uuid);

	/**
	 * Provides a callback to invoke and provide a UUID for when an invalidation
	 * is received from across the network, regardless of whether we are
	 * supposed to receive it or not. So if we don't have a player with this
	 * UUID on our server then we still should give it to the consumer. If
	 * <code>null</code> is passed into this method then nothing should be done
