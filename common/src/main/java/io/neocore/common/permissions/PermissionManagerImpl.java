...
public class PermissionManagerImpl implements PermissionManager {
	...
	public void assignPermissions(NeoPlayer player) {
		...
		for (GroupMembership gm : dbp.getGroupMemberships()) {
			...
			if (g == null) {
				...
				NeocoreAPI.getLogger().warning("Found an invalid group in a membership when processing permissions for "
						+ player.getUniqueId() + ".");
				continue;
		 ...
		 * we're going to start with the group with lowest priority. Here, we'll
		 * still be doing things in order, so if we get a state for a node
		 * *after* we've already got a setting, we'll override the value in the
		 * map. Which is what we want because we'll be sure that the group that
		 * the assignment comes from is of a higher priority.
		 */
	...
	public void repopulatePermissions() {
		...
		NeocoreAPI.getLogger()
				.info("Took " + (end - start) + " ms to fully reload permissions for " + nps.size() + " players.");
