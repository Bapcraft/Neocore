...
public class CommonEventManager extends EventManager {
	...
	@Override
	public <T extends Event> RegisteredListener<T> registerListener(Module mod, Class<?> type, Consumer<T> listener, int priority) {
		
		Preconditions.checkNotNull(mod);
		Preconditions.checkNotNull(type);
		Preconditions.checkNotNull(listener);
		Preconditions.checkArgument(Event.class.isAssignableFrom(type), "You did not pass an child of " + Event.class.getName() + "!");
		...
		for (Method m : methods) {
			...
			if (m.isAnnotationPresent(NeocoreEventHandler.class)) {
				...
				if (m.getParameterCount() != 1) {
					...
					log.warning("Method " + clazz.getName() + "." + m.getName() + " has an @" + NeocoreEventHandler.class.getSimpleName() + " annotation on it, but there is not 1 parameter.");
					continue;
				...
				if (!Event.class.isAssignableFrom(type) || !type.isAnnotationPresent(Raisable.class)) {
					...
					log.warning("Method " + clazz.getName() + "." + m.getName() + " is doesn't take an instance of " + Event.class.getName() + " with the @" + Raisable.class.getSimpleName() + " annotation on it.");
					continue;
