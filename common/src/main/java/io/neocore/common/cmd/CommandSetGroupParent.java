...
public class CommandSetGroupParent extends AbstractCommand {
	...
	public void onExecute(CmdSender sender, String[] args) {
		...
		if (!sender.hasPermission("neocore.cmd.permissions.setparent"))
			this.noPerms();
		if (args.length != 2)
			this.badUsage();
		...
		if (sender.isPlayer()) {
			...
			if (np.hasIdentity(DatabasePlayer.class)) {
				...
				if (group.getRestrictionLevel() > level || (parent != null && parent.getRestrictionLevel() > level)) {
					...
					if (parent != null)
						sender.sendMessage(" - " + parent.getDisplayName() + ": " + parent.getRestrictionLevel());
				...
				sender.sendMessage(
						"You don't have a database identity active on this server.  Check the server's status.");
			}
