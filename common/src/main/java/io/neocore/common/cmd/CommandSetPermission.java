...
public class CommandSetPermission extends AbstractCommand {
	...
	public void onExecute(CmdSender sender, String[] args) {
		...
		if (!sender.hasPermission("neocore.cmd.permissions.setperm")) this.noPerms();
		...
		// We have to verify restriction levels if the thing running this is a player.
		if (sender.isPlayer()) {
			...
			if (np.hasIdentity(DatabasePlayer.class)) {
				...
				sender.sendMessage("We can't verifiy your permission level due to problems that we already know about.");
				this.badUsage();
