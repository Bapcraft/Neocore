...
public class CommandActiveUserManager extends AbstractCommand {
	...
	public void onExecute(CmdSender sender, String[] args) {
		...
		if (!sender.hasPermission("neocore.cmd.activeusers"))
			this.noPerms();
		...
		for (Map.Entry<UUID, List<PlayerLeaseImpl>> entry : leases.entrySet()) {
			...
			sender.sendMessage(
					String.format(" - %s (%s) [%s] %s", username, np.getUniqueId().toString().substring(0, 8),
							np.isPopulated() ? "OK" : "LOADING", sdf.format(np.getConstructionTime())));
			...
			if (!pls.isEmpty()) {
				...
				StringBuilder sb = new StringBuilder("   - Leases (" + pls.size() + "): " + pl0.isValid() + "["
						+ sdf.format(pl0.getIssueTime()) + "]");
