...
public class ServiceManagerImpl implements ServiceManager {
	...
	public List<ServiceType> getUnprovidedServices() {
		...
		// Loop through the services and see which there are to deal with. This
		// funny loop logic is to avoid checking types we've already ruled out.
		for (RegisteredServiceImpl sr : this.services) {
			...
			while (typesRemaining.hasNext()) {
				...
				if (type.getServiceClass() != null
						&& type.getServiceClass().isAssignableFrom(sr.getServiceProvider().getClass()))
					typesRemaining.remove();
	...
	public void registerServiceProvider(Module mod, ServiceType type, ServiceProvider service) {
		...
		if (this.locked)
			throw new IllegalStateException("Cannot add service, already initialized!");

		if (service == null) {

			NeocoreAPI.getLogger()
					.warning("Tried to load a null service from module " + mod + " for type " + type.getName());
			return;
		...
		if (!typeClazz.isAssignableFrom(clazz))
			throw new ClassCastException("The class " + clazz + " is not an instance of " + typeClazz + "!");
		...
		for (Entry<Class<? extends ServiceProvider>, List<ServiceRegistrationHandler>> e : this.registrationHandlers
				.entrySet()) {
			if (e.getKey().isAssignableFrom(service.getClass())) {
				for (ServiceRegistrationHandler handler : e.getValue())
					handler.onRegister(rs);
			}
		}
	...
	public <T extends ServiceProvider> T getService(Class<T> servClazz) {
		...
		for (RegisteredServiceImpl rs : this.services) {
			if (servClazz.isAssignableFrom(rs.getServiceProvider().getClass()))
				return (T) rs.getServiceProvider();
		}
	...
	public void registerRegistrationHandler(Class<? extends ServiceProvider> servClazz,
			ServiceRegistrationHandler handler) {
	...
	public synchronized void initializeServices() {
		...
		if (this.locked)
			throw new IllegalStateException("Tried to initialize services that are already initialized!");
