...
public class PlayerManagerWrapperImpl implements PlayerManager {
			...
			if (plis.size() == 0) {
				...
				this.playerManager.flushPlayer(uuid, FlushReason.OTHER, () -> {
					...
					this.playerManager.unloadPlayer(uuid, UnloadReason.OTHER, () -> {
						NeocoreAPI.getLogger().fine("Unloaded player " + uuid + " (hashcode: " + np.hashCode() + ") from Neocore.");
					});
	...
	public synchronized PlayerLease requestLease(UUID uuid, Consumer<NeoPlayer> callback) {
		...
		NeocoreAPI.getLogger().fine("Lease for " + uuid + " requested, for a total of " + playerLeases.size() + " leases.");
		
		// Actually start construction.
		if (!this.isInited(uuid)) this.playerManager.assemblePlayer(uuid, LoadReason.OTHER, this::flagLeases);
		
		// If it's okay then flag it already
		if (this.isPopulated(uuid)) lease.flagCompleted();
