...
public class CommonPlayerManager {
	...
	public void overrideNetworkSync(NetworkSync override) {
		...
		// Cleanup the old one.
		this.networkSync.updatePlayerList(new HashSet<>()); // Close out any
															// inbound
															// connections.
	...
	private void processInvalidation(UUID uuid) {
		...
		if (player != null) {
			...
			if (player.isPopulated()) {
				...
				log.warning(
						"Player hasn't finished being populated, but we tried to revalidate it.  Queueing delayed thread to do it.");
				this.scheduler.invokeAsync(() -> { // We don't make this get
													// wrapped by
													// invokeAccordingToModel
													// because this has to be
													// done by forking away from
													// the network thread
													// anyways.
	...
	private void wrapServices() {
		...
		NeocoreAPI.getLogger()
				.fine("Wrapping " + this.loadableServices.size() + " services for player dependency injection.");
	...
	private void updateContainerLockCoordinators() {
		...
		for (ProviderContainer pc : this.providerContainers) {
			if (pc instanceof LockableContainer)
				((LockableContainer) pc).overrideLockCoordinator(this.networkSync.getLockCoordinator());
		}
	...
	private void addServiceWrapper(ServiceType type) {
		...
		if (!IdentityProvider.class.isAssignableFrom(type.getServiceClass())) {
			...
			NeocoreAPI.getLogger()
					.warning("Tried to wrap an identity service that wasn't actually an identity service! ("
							+ type.getServiceClass().getName() + ")");
			return;
		...
		IdentityProvider<?> identProvider = (IdentityProvider<?>) this.serviceManager.getService(type)
				.getServiceProvider();
	...
	private void addCachedPlayer(NeoPlayer np) {
		...
		if (np != null) {
			...
			NeocoreAPI.getLogger()
					.finest("Cached NeoPlayer " + np.getUniqueId() + " (hashcode: " + np.hashCode() + ").");
	...
	private void removeCachedPlayer(NeoPlayer np) {
		...
		NeocoreAPI.getLogger()
				.finest("Removing NeoPlayer " + np.getUniqueId() + " (hashcode: " + np.hashCode() + ") from cache.");
		if (!this.playerCache.remove(np))
			NeocoreAPI.getLogger().warning("Tried to remove NeoPlayer " + np.getUniqueId() + " (hc" + np.hashCode()
					+ ") from cache, but it wasn't in the cache!");
	...
	private void invokeAccordingToModel(Runnable r) {
		...
		if (this.ioModel == PlayerIoThreadingModel.SINGLE_THREAD) {
			...
			this.scheduler.invokeAsync(r);
				...
				|| this.ioModel == PlayerIoThreadingModel.AUTO_DEFERRED) {

			// Not the best way to do it but do to the way things are ordered
			// right now it's the best for how we're using this method.
	...
	public synchronized NeoPlayer assemblePlayer(UUID uuid, LoadReason reason, Consumer<NeoPlayer> callback) {
		...
		if (this.wrappersDirty)
			this.wrapServices();
		this.eventManager.broadcast(new PreLoadPlayerEvent(reason, uuid));

		if (reason == LoadReason.JOIN || reason == LoadReason.OTHER)
		...
		for (ProviderContainer container : this.providerContainers) {
			...
			ProvisionResult result = container.load(np, res -> {
				...
				if (res.getStatus() != ActionStatus.SUCCESS) {
					log.log(Level.SEVERE,
							"Problem loading identity: " + container.getProvider().getIdentityClass().getSimpleName(),
							res.getException());
				}
			...
			log.finer(String.format("Provision result for %s (hashcode: %s) on %s was %s.", np.getUniqueId(),
					np.hashCode(), container.getProvider().getClass().getSimpleName(), result.name()));

		}

		// Now wait for all of the containers to finish their work before
		// calling back.
		this.invokeAccordingToModel(() -> {
			...
			if (callback != null)
				this.invokeAccordingToModel(() -> callback.accept(np));
		...
		// Now return the container while it gets populated in some other
		// thread.
		return np;
	...
	private void flushPlayerBlockingly(NeoPlayer np) {
		...
		try {
			...
			boolean flushOk = flushWaiter.await(15000L, TimeUnit.MILLISECONDS); // FIXME
																				// Make
																				// configurable.
			if (!flushOk) {
	...
	public synchronized void flushPlayer(UUID uuid, FlushReason reason, Runnable callback) {
		...
		if (np == null)
			throw new IllegalArgumentException("This player doesn't seem to be loaded! (" + uuid + ")");
		...
		for (ProviderContainer container : this.providerContainers) {
			...
			if (ident != null) {
				...
				log.fine("In the process of flushing " + np.getUniqueId() + ", we skipped "
						+ container.getProvisionedClass().getSimpleName() + " of "
						+ container.getProvisionedClass().getSimpleName() + ".");
				latch.countDown();
		...
		// Spawn the thread that invokes the callback once everything's
		// unloaded.
		this.invokeAccordingToModel(() -> {

			try {

				boolean flushOk = latch.await(10000L, TimeUnit.MILLISECONDS); // FIXME
																				// Make
																				// configuratble.
				if (!flushOk) {
				...
				log.warning(
						"Waiting for identity flushing was interrupted, broadcasting events and invoking callback anyways...");
			} finally {
	...
	public synchronized void unloadPlayer(UUID uuid, UnloadReason reason, Runnable callback) {
		...
		for (ProviderContainer container : this.providerContainers) {
			...
			if (ident != null) {
				...
				container.unload(np, res -> {
					...
					if (res.getStatus() != ActionStatus.SUCCESS) {
						log.log(Level.SEVERE,
								"Problem unloading player " + uuid + ", apparently: " + res.getStatus().name());
					}
		...
		this.invokeAccordingToModel(() -> {
			...
			try {
				...
				if (reason == UnloadReason.DISCONNECT)
