...
public class ClientAcceptWorker implements Runnable {
	...
	/**
	 * Tells the loop to stop accepting connections. This will wait until the
	 * next loop timeout.
	 */
	public void finish() {
	...
	public void run() {
		...
		while (this.accepting) {
			...
			try {
				...
				Nmd.logger.info(String.format("New connection from %s:%s!  Initializing...",
						added.getInetAddress().getHostAddress(), added.getPort()));
