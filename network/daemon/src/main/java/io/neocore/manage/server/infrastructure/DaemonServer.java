...
public class DaemonServer {
	...
	public DaemonServer(Scheduler sched, ServerSocket socket, int threadCount) {
		...
		this.messageManager.registerHandler(PayloadCase.PING, new PingHandler());
				...
				() -> {
					...
					try {
						...
						if (type == ClientType.SERVER) {
							...
							Nmd.logger.info("New client " + nmc.name + " connected from "
									+ nmc.socket.getInetAddress().getHostAddress() + ".");
	...
	private void register(NmClient client) {
		...
		if (this.clients.contains(client))
			throw new IllegalArgumentException("Already have this client registered!");
	...
	public void unregister(NmClient client) {
		...
		if (this.clients.contains(client)) {
			...
			this.remove(client);
			Nmd.logger.info(
					"Client " + client.getIdentString() + " from " + client.getAddressString() + " disconnected.");

		} else {
			throw new IllegalArgumentException(
					"Could not find client " + client.getIdentString() + ".  Is it already unregistered?");
		}
