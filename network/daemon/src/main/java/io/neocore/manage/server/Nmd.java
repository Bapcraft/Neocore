...
public class Nmd {
	...
	public synchronized void start() {
		...
		if (this.isStarted())
			throw new IllegalStateException("NMD already started!");
		...
		while (ok) {
			...
			if (line.equals("clients")) {
				...
				for (NmClient nmc : instance.server.getClients()) {
					...
					logger.info("    - " + nmc.getAddress().getHostAddress() + ":" + nmc.getPort() + " ("
							+ nmc.getAddress().getHostName() + ")");
					logger.info(
							"    - Last updated " + (System.currentTimeMillis() - nmc.getLastUpdated()) + " ms ago");
