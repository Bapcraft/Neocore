...
public class NmNetworkMap implements NetworkMap {
	...
	private NmNetworkPlayer findPlayer(UUID uuid) {
		...
		for (NmNetworkPlayer nnp : this.players) {
			if (nnp.getUniqueId().equals(uuid)) return nnp;
		}
	...
	public NmNetworkPlayer addPlayerTo(UUID uuid, NmNetworkComponent component) {
		...
		if (component instanceof NmEndpoint && !this.endpoints.contains(component)) throw new IllegalArgumentException("Tried to add a player to an endpoint not in its network.");
	...
	private void purgeAbsentPlayers() {
		...
		while (iter.hasNext()) {
			...
			if (this.getFrontend_Nm().hasPlayer(nnp.getUniqueId())) count++;
			for (NmEndpoint ne : this.getEndpoints_Nm()) {
				if (ne.hasPlayerId(nnp.getUniqueId())) count++;
			}
			
			if (count == 0) iter.remove();
