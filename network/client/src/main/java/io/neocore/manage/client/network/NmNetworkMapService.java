...
public class NmNetworkMapService implements NetworkMapService {
	...
	public NmNetworkMap getNetworkByName(String name) {
		...
		for (NmNetworkMap dnm : this.maps) {
			if (dnm.getNetworkName().equals(name))
				return dnm;
		}
	...
	public void registerFrontend(NmFrontend front) {
		...
		if (this.getNetworkByMemberId(front.getAgentId()) != null)
			throw new IllegalArgumentException("Already registered frontend " + front.getAgentName() + ".");
		...
		if (map == null && front instanceof NmProxyFrontend) {
			...
			throw new IllegalArgumentException("Network " + front.getNetworkName()
					+ " already has a frontend registered, " + map.getFrontend().getAgentName() + ".");
		}
	...
	public void registerEndpoint(NmEndpoint endpoint) {
		...
		if (this.getNetworkByMemberId(endpoint.getAgentId()) != null)
			throw new IllegalArgumentException("Already registered endpoint " + endpoint.getAgentName() + ".");
		NmNetworkMap map = this.getNetworkByName(endpoint.getNetworkName());

		// We are sure we haven't already added it because of that conditional
		// above.
