...
public class NmServer implements Closeable {
	...
	public void queueMessage(ClientMessage message) {
		...
		if (this.lastMessageTime + this.timeout < System.currentTimeMillis()) {
			...
			NeocoreAPI.getLogger().severe("Tried to queue a message to " + this.getLabel() + ", but we haven't recieved a message from them in " + dur + " ms, vs timeout of " + this.timeout + "!");
			this.close();
			
		} else {
			
			this.queue.add(message);
			NeocoreAPI.getLogger().finer("Queued message ID:" + Long.toHexString(message.getMessageId()) + " to server " + this.getLabel() + ".");
	...
	public synchronized void close() {
		...
		try {
			...
			b.setUnregClient(
					UnregisterClient
						.newBuilder()
						.setReason(UnregisterReason.SHUTDOWN)
						.setReasonStr("thx bb <3")
						.build());
