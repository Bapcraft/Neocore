...
/**
 * Represents human-readable properties about the context of the host. A host
 * can have multiple simultaneous contexts.
 * 
 * @author treyzania
 */
public interface Context {
	...
	static final List<Context> createdContexts = new ArrayList<>(Arrays.asList(GLOBAL)); // Can't
																							// define
																							// initalizers
																							// so
																							// this
	 ...
	 * list of already-created contexts. If a <code>null</code> is passed, then
	 * the GLOBAL context is returned.
	 * 
	 * @param name
	...
	public static Context create(String name) {
		...
		for (Context ctx : createdContexts) {
			if (ctx != null && ctx.getName() != null && ctx.getName().equals(name))
				return ctx;
		}
	...
	public static boolean checkCompatility(Context env, Context perm) {
		...
		if (env == null || perm == null)
			return false;
		if (perm == GLOBAL || env == GLOBAL)
			return true;
